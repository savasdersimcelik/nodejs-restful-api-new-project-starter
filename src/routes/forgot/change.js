...
const route = async (req, res) => {
    ...
    const _user = await user.findOne({ _id: decrypt_key._id, forgot_key: body.key }).select('+password');
    if (!_user) {
    ...
    const data_stringify = JSON.stringify({ _id: _user._id });                  // Datalar JSON formatına dönüştürülüyor.
    ...
    _user.set({                                                                 // Kullanıcı dataları set ediliyor.
        forgot_key: forgot_key.toString(),                                      // Yeni forgot_hash değeri
